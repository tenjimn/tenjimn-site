...
| 8 | `remove_hr_after_headings` | h2/h3 直後の `---` を除去（CSSと二重線になるため） |
| 9 | `convert_csv_gallery` | Notion DBの CSVリンクをキャプショングリッドHTMLに変換 |
| 10 | `process_images` | 画像をコピー、パスを書き換え、HEIC→JPEG変換 |
...
### CSVギャラリー
NotionのDBギャラリーはCSV+サブフォルダとしてエクスポートされる。画像とキャプションの紐づけはエクスポート時に消えるため、キャプション一覧のみグリッド表示。
