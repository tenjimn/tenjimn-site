...
大前提として、スクリプトによる変換後のマークダウンを直接編集することは禁止とした。万が一過去記事に再変換をかけた際に、conflictしてしまうのを防ぐためである。表示がおかしい場合は、必ずスクリプト汎用変換ロジック側を修正する、という運用ルールを徹底した。加えて、今後の保守性やAIエディタ主導での開発を考えると、プロジェクトの設計方針や変換ルールを`AGENTS.md`のドキュメントとして明文化し、AIエージェントに読ませる状態を作っておくことが非常に大切だと実感している。
...
## <strong><strong>更なるデジタルアーカイブを再構築する</strong></strong>
